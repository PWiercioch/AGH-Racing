...
Generalizing a damper can be reperesented as a spring. A force acting on a spring is given by formula: spring displacement times spring constat. Using linear potetniometers we logged potentiometers lengths. Knowning the spring constant as well as motion ratio(similar to gear ratio only made by suspension rods instead of gears) we can calculate force acting on a car. Except for direct load forces acting on a car are:
- aerodynamic downforce
- inertial forces
A full suspension system (in our case) consists of 4 dampers: 2 in the front and 2 in the back. One of each is a heave damper and the other is roll damper. Heave damper is activated by vertical dispalcement eg. during breaking when car is leaning forawrd. Roll damper is activated by a horizontal dispalcement (rotation) eg. when car is leaning outwards during cornering. Potentiometers output voltage which can be easily converted to damper length. The more damper is compressed (shorter) the more force is acting on it.
Detailed explanaiton on how a suspension system similar to one used in our car can be found [here](https://www.youtube.com/watch?v=8js55jE2da0).

Format of data logged changed continuosly throughout the season and scripts used for analysis had to change as well. In this study four scripts were used.
...
It can be seen that the initial values for potentiometer lengths have changed. Also there is no histeresis error the *Test 1* (potentiometers return to their initaial position).

## Track data analysis
The script ***Calculate_forces.m*** was used to analise data collected during track tests. 
...
Next from logged potentiometers lengths relative displacement is calculated - first entry is treated as a reference value (assumption that data wa started being recorded while the car was staionary). From relative displacement elocity can be calculated also. From data errors (weird peaks) are filtered out by replacing velocity higher than thershold by a previous correct value.
...
When car slightly rotates to the outer side during a corner a mass transfer occures - more weight of the car is put on one side. With our collected data we couldnt specify whether car is rotating to the left or to the right but we could calcualte the ratio beetwen loads on tires for each side. Data is plotted for ***heavy loaded tire*** and ***light loaded tire***. 
...
One way to measure downforce acting on a car is to drive at constant speed on a straight line so no acceration is acing upan a car:
- no longitudonal acceleration - constant speed
...
- roll dampers are decompresing - lateral acceleration is lowering
- heave dampers are compresing - the car is speeding up
